...
if model_tamp_attack_level > 0:
    if 'DQN' not in mode or agent is None:
        raise ValueError(
            "ModelTampAttack_level > 0 requires a DQN/DDQN agent because the attack tampers with the running model parameters."
        )
    from mdp_attacks import install_model_tamp_attack
    model_tamp_attack_profile = install_model_tamp_attack(model_tamp_attack_level, agent)
    print(
        "Model tamper attack enabled:",
        {
            'level': model_tamp_attack_level,
            'apply_probability': model_tamp_attack_profile['apply_probability'],
            'hidden_epsilon': model_tamp_attack_profile['hidden_epsilon'],
            'output_epsilon': model_tamp_attack_profile['output_epsilon'],
            'value_epsilon': model_tamp_attack_profile['value_epsilon'],
            'stealth_clip_ratio': model_tamp_attack_profile['stealth_clip_ratio'],
        }
    )
