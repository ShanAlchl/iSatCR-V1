...
- security evaluation through multiple attack mechanisms in [`iterative_testing/mdp_attacks`](./iterative_testing/mdp_attacks)

## Main Files

- [`iterative_testing/PRC.py`](./iterative_testing/PRC.py): main training and test entry
- [`iterative_testing/RL_environment_for_computing.py`](./iterative_testing/RL_environment_for_computing.py): RL environment wrapper and metric logging
- [`iterative_testing/SatelliteNetworkSimulator_Computing.py`](./iterative_testing/SatelliteNetworkSimulator_Computing.py): satellite network simulation core
- [`iterative_testing/Base_Agents.py`](./iterative_testing/Base_Agents.py): DQN, DDQN, and PPO agent definitions
- [`iterative_testing/Draw_Graph_Quiker.py`](./iterative_testing/Draw_Graph_Quiker.py): satellite topology visualization
- [`iterative_testing/Make_Satellite_Graph.py`](./iterative_testing/Make_Satellite_Graph.py): graph construction utilities
- [`train`](./train): YAML configuration files
- [`iterative_testing/mdp_attacks`](./iterative_testing/mdp_attacks): attack modules
...
| Script | Description |
| --- | --- |
| [`mdp_StateObservation_attack.py`](./iterative_testing/mdp_attacks/mdp_StateObservation_attack.py) | Adversarial perturbation on observed state vectors |
| [`mdp_action_attack.py`](./iterative_testing/mdp_attacks/mdp_action_attack.py) | Tampering with executed actions |
| [`mdp_Reward_attack.py`](./iterative_testing/mdp_attacks/mdp_Reward_attack.py) | Reward shaping or tampering before experience storage |
| [`mdp_StateTransfer_attack.py`](./iterative_testing/mdp_attacks/mdp_StateTransfer_attack.py) | Poisoning transition tuples before replay |
| [`ExperiencePool_attack.py`](./iterative_testing/mdp_attacks/ExperiencePool_attack.py) | Replay-buffer poisoning during update |
| [`ModelTamp_attack.py`](./iterative_testing/mdp_attacks/ModelTamp_attack.py) | Runtime model parameter tampering |
...
```bash
python iterative_testing/PRC.py --config train/train_NewDDQN_dueling_shuffle.yaml
```
...
For constellation configuration `2`, independent mode can optionally use region-level agents to avoid creating one model per satellite. This is conservative and is disabled unless explicitly configured:

```yaml
agent:
  agent_sharing_mode: "independent"
  enable_region_agent_for_large_constellation: true
  region_agent_constellation_ids: [2]
  region_orbit_block_size: 5
  region_sat_block_size: 5
  intra_region_routing: "dijkstra"
```

With `ConstellationConfig: 2`, the 60x60 satellite grid is split into 5x5 blocks, producing up to 12x12 = 144 `Region_<altitude>_<orbit_block>_<sat_block>` agents. Satellites in the same region share the same independent agent/checkpoint. Routing inside one region uses Dijkstra on the current region subgraph and is not written to the RL replay buffer; routing across regions keeps the original RL neighbor-action space and writes experience to the region agent. When action logging is enabled, region-mode RL/fallback records add `routing_scope` (`intra_region`/`inter_region`) and `routing_policy` (`rl`/`fallback_rl`) fields.
...
| `agent.enable_region_agent_for_large_constellation` | optional `false` by default; when `true` with `ConstellationConfig: 2` and independent mode, use region-level agents |
| `agent.region_orbit_block_size`, `agent.region_sat_block_size` | region grid block sizes; use `5` and `5` for 144 regions in constellation `2` |
| `agent.intra_region_routing` | intra-region traditional routing method; currently supports `dijkstra` |
...
The training and test logs print the following 11 metrics once per statistics window. Their definitions follow the implementation in [`iterative_testing/RL_environment_for_computing.py`](./iterative_testing/RL_environment_for_computing.py).
...
The traffic-generation path in [`iterative_testing/SatelliteNetworkSimulator_Computing.py`](./iterative_testing/SatelliteNetworkSimulator_Computing.py) now tolerates temporary cases where the currently selected destination side has no visible satellite candidate:
...
The project includes topology visualization tools in [`iterative_testing/Draw_Graph_Quiker.py`](./iterative_testing/Draw_Graph_Quiker.py):
