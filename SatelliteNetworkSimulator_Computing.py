    ...
    def forward_packet(self):
        while self.active:
            ...
            if last_state is not None and reward is not None:
                self.propagator.append_experience(self.name, last_state, current_state[-1], last_action, reward, current_state, done, 'routing')
            ...
            while self.env.now < end_time:
                ...
                if self.select_mode != 1:
                    ...
                    if destination is None:
                        self.logger.log(f"Time {self.env.now:.3f}: no visible satellite is currently available for {satellite}, retry next packet generation.")
                        continue
                    if previous_destination is not None and destination != previous_destination:
                        print("*****", flush=True)
                    self._update_traditional_neighbor_graph(satellite, destination, min_hops)
