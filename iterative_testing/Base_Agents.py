...
class DDQN_Agent:
    ...
    def load_model(self, file_path):
        if file_path:
            state_dict = torch.load(file_path, map_location=self.device, weights_only=False)
...
class DQN_Agent:
    ...
    def load_model(self, file_path):
        if file_path:
            self.online_net.load_state_dict(torch.load(file_path, map_location=self.device, weights_only=False))
...
class PPO_Agent:
    ...
    def load_model(self,file_path):
        if file_path:
            self.online_net.load_state_dict(torch.load(file_path + '/actor.pth', map_location=self.device, weights_only=False))
            self.critic_net.load_state_dict(torch.load(file_path + '/critic.pth', map_location=self.device, weights_only=False))
