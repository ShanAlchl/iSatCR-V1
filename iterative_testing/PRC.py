...
def parse_args():
    parser = argparse.ArgumentParser(description="Run the satellite simulation with specified configuration file.")
    parser.add_argument('--config', type=str, default='train/train_NewDDQN_dueling.yaml',
...
def list_constellation_tle_paths():
    satellite_data_dir = PROJECT_ROOT / 'Satellite_Data'
...
def resolve_train_bootstrap_model_path(config):
    ...
    pretrained_root = Path(agent_config.get('pretrained_model_root', os.path.join('.', 'model_weights')))
