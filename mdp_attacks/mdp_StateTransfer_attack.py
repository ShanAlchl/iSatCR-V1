...
class StateTransferAttackEngine:
    ...
    def maybe_attack(self, propagator, agent_name, state, mark, action, reward, next_state, done, event_type):
        ...
        history_key = (agent_name, int(clean_state.shape[0]))
        ...
        if not self.enabled:
            self._update_transition_history(history_key, clean_state, clean_next_state)
        ...
        reference_delta = self._get_reference_delta(history_key, clean_state, clean_next_state)
    ...
    def _get_reference_delta(self, history_key, clean_state, clean_next_state):
        clean_delta = clean_next_state - clean_state
        historical_delta = self._transition_history.get(history_key)
        if historical_delta is None:
            return clean_delta
        return 0.5 * clean_delta + 0.5 * historical_delta

    def _update_transition_history(self, history_key, clean_state, clean_next_state):
        decay = float(self.profile["history_decay"])
        clean_delta = clean_next_state - clean_state
        if history_key not in self._transition_history or decay <= 0:
            self._transition_history[history_key] = clean_delta.copy()
            return
        self._transition_history[history_key] = (
            decay * self._transition_history[history_key] + (1.0 - decay) * clean_delta
        )
